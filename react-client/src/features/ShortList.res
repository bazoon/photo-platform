...
let make = (~id: string) => {
  ...
  let loadSection = (s: option<section>) => {
    ...
    setSection(_ => s)
    asyncGetImages(`api/jury/images/${id}`).fork(failed, okImages, cleanUp)
  }
  ...
  let move = (dir: int) => {
      setSections((s: array<section>) => {
        setImageIndex(old => {
          ...
          setSection(_ => s -> Belt.Array.get(newIndex))
          newIndex
      })
      s
    })
  ...
  let renderThumb = (image: image) => {
    let cls = "object-contain w-full h-full"
    <div className="grid grid-cols-2 gap-10">
      <div className="w-auto h-52"><img src={image.filename} className={cls}/></div>
      <div className="h-52 cursor-pointer flex-shrink-0 flex-grow-0 flex-col flex">
        ...
        <span className="text-sm-2">{React.string(image.description)}</span>
      </div>
    </div>
  };
